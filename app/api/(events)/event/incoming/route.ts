...
export const GET = async (request: NextRequest) => {
  try {
    ...
    const start_date = request.nextUrl?.searchParams.get("event_id") ?? null;
    ...
    const response = await queryIncomingEvents("EventDetails", lastEvaluatedKey as unknown as Record<string, AttributeValue>);
    return response;
  } catch (error) {
    return error;
  }
