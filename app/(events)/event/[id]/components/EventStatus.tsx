...
function getEventStatus({ start_date, end_date }: EventDetails) {
    ...
    if (now.isBefore(startDate)) {
        return { status: 'Upcoming', className: 'bg-red-100 text-red-500 font-medium' };
    } else if (now.isBetween(startDate, endDate)) {
        return { status: 'Ongoing', className: 'bg-green-400 text-white font-medium' };
    } else {
        return { status: 'Finished', className: 'bg-neutral-500 text-white font-medium' };
    }
}
